...
import {
  LI_TABS, contactById, lastInboundFrom, liConversations, liThreadFor, liWaiting,
  suggestedReply, threadsFor,
...
/* --------------------------------------------------------- profile URLs */
{
  ...
  ok("every seeded contact has an openable profile",
    S.contacts.every((c) => profileUrl(c.linkedin) !== null),
    S.contacts.filter((c) => !profileUrl(c.linkedin)).map((c) => c.linkedin));

  const added = run(S, { type: "addContact", contact: blankContact("Zoe Zed", "Zed", "MD", "z@zed.com", "") });
  ok("a contact with no URL is not openable", profileUrl(added.contacts[15].linkedin) === null);
}
...
/* ------------------------------------------------------ chat committing */
{
  const s1 = run(S, { type: "liLogChat", cid: 3, messages: [{ dir: "in", body: "Sure, what did you have in mind?" }] });
  const t = liThreadFor(s1, 3)!;
  ...
  const auto = suggestedReply(s1, contactById(s1, 3)!);
  ...
  const again = run(s1, { type: "liRegenerate", cid: 3 });
  ok("another angle is different", suggestedReply(again, contactById(again, 3)!) !== auto);
  ok("regenerate says so", /different angle/.test(again.toast), again.toast);

  const dropped = run(s1, { type: "liDismissDraft", cid: 3 });
  ok("discard clears it", suggestedReply(dropped, contactById(dropped, 3)!) === "");

  ok("nothing is drafted before they write", suggestedReply(S, contactById(S, 3)!) === "");
  ok("an accepted connection is offered an opener",
    suggestedReply(S, contactById(S, 6)!).length > 40, suggestedReply(S, contactById(S, 6)!));
  ok("AI refuses to shorten an empty box", generateLi(contactById(s1, 3)!, "shorter", "", "") === null);

  const s2 = run(s1, { type: "liLogChat", cid: 3, messages: [{ dir: "out", body: "Fifteen minutes on Tuesday?" }] });
  const t2 = liThreadFor(s2, 3)!;
  ...
  const s3 = run(S, { type: "liLogChat", cid: 1, messages: [{ dir: "out", body: "Thanks for connecting." }] });
  ...
  const s4 = run(S, { type: "liLogChat", cid: 7, messages: [{ dir: "in", body: "Following up." }] });
  ...
  const s5 = run(S, { type: "liLogChat", cid: 3, messages: pasted });
  const t5 = liThreadFor(s5, 3)!;
  ...
  ok("a transcript ending on their turn drafts a reply", suggestedReply(s5, contactById(s5, 3)!).length > 40);
  ok("adding your own message leaves no draft", suggestedReply(s3, contactById(s3, 1)!) === "");
  ok("empty paste is a no-op", run(S, { type: "liLogChat", cid: 3, messages: [] }) === S);
